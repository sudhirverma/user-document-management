...
export class UsersDocumentRepository implements UserRepository {
  ...
  async create(data: User): Promise<User> {
    const persistenceModel = toPersistence(data);
    const createdUser = new this.usersModel(persistenceModel);
    const userObject = await createdUser.save();
    return toDomain(userObject);
  }
  ...
  async findManyWithPagination({
    ...
    return userObjects.map((userObject) => toDomain(userObject));
  }

  async findById(id: User['id']): Promise<NullableType<User>> {
    const userObject = await this.usersModel.findById(id);
    return userObject ? toDomain(userObject) : null;
  }
